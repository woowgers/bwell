...
from bwell.forms.forms import OrderFilterForm
...
def admins():
    if "orders" not in session:
        return render_template(
            "admins_orders.j2",
            orders=db_get_admins_orders(db),
        )
    else:
...
def admin_orders(user_id: int):
    return render_template(
        "admin_orders.j2",
        orders=db_get_admin_orders(db, user_id),
    )
...
def customers_orders():
    if "customers_orders" not in session:
        return render_template(
            "customers_orders.j2",
            orders=db_get_customers_orders(db),
        )
    else:
...
def customer_orders(user_id: int):
    return render_template(
        "customer_orders.j2",
        orders=db_get_customer_orders(db, user_id),
    )
