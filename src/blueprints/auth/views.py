...
def my():
    ...
    delete_form = Form(
        action=url_for("account.delete_my", user_id=user.user_id),
        submit_value="Delete Account",
    )
    return render_template(
        "account.j2", logout_form=logout_form, delete_form=delete_form
    )
...
def all():
    def delete_form(user_id: int) -> Form:
        return Form(
            submit_value="Delete", action=url_for("account.delete", user_id=user_id)
        )
...
def delete_my(user_id: int):
    if user_id != user.user_id:
        abort(406, f"You are not allowed to delete this user")
